...
func weather() {
	...
	image := canvas.NewImageFromFile("./img/iweather.png")
	app := fyne.CurrentApp().NewWindow("Weather App By Jahanwee")

	cityList := [][2]string{
		{"Delhi", "delhi"},
		{"Noida", "noida"},
		{"Mumbai", "mumbai"}}
		mycon:= container.NewMax(image ,makeListTab(cityList , app))

	app.SetContent(
		mycon,
	)
	app.Resize(fyne.NewSize(200,400))
	app.Show()
}
...
func makeListTab(data [][2]string, app fyne.Window) fyne.CanvasObject {
	...
	r := container.NewHSplit(list, hbox)   // horizontal split the window
	r.Offset = 0       // for split container
	return r
}
